    ...
    def run(self, workflow):
        ...
        if ExaComm.is_learner():
            ...
            while episode_done < workflow.nepisodes:
                ...
                target_weights = workflow.agent.get_weights()
                agent_comm.send([worker_episodes[whofrom - 1], epsilon, target_weights], whofrom)
            ...
            while episode != -1:
                ...
                while steps < workflow.nsteps:
                    ...
                    if ExaComm.is_actor():
                        ...
                        pack = not hasShape
                        agent_comm.send([agent_comm.rank, steps, batch, policy_type, done], 0, pack=pack)

                        logger.info('Rank[%s] - Episode/Step/Status:%s/%s/%s' % (str(agent_comm.rank), str(episode), str(steps), str(done)))
