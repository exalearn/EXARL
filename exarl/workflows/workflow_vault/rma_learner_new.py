...
class RMA_NEW(exarl.ExaWorkflow):
    ...
    def learner(self, workflow, learner_counter, episode_const, epsilon_const, ind_loss_buffer, model_buff, batch_data_exchange, agent_data):
        # Initialize counter
        ...
        while True:
            ...
            if ExaComm.num_learners > 1:
                episode_count_learner = ExaComm.learner_comm.bcast(episode_count_learner, root=0)
            ...
            if agent_data is None:
                agent_data, actor_idx, actor_counter = batch_data_exchange.get_data(
                    learner_counter, ExaComm.learner_comm.size, ExaComm.agent_comm.size)
            ...
            sum_process_has_data = ExaComm.learner_comm.allreduce(process_has_data, op=sum)
            if sum_process_has_data < ExaComm.learner_comm.size:
                continue
        ...
        logger().info('Learner exit on rank_episode: {}_{}'.format(ExaComm.agent_comm.rank, episode_count_learner))

    def actor(self, workflow, episode_const, epsilon_const, ind_loss_buffer, model_buff, batch_data_exchange):
        local_actor_episode_counter = 0
        if ExaComm.env_comm.rank == 0:
            # Logging files
            filename_prefix = 'ExaLearner_' + 'Episodes%s_Steps%s_Rank%s_memory_v1' \
                % (str(workflow.nepisodes), str(workflow.nsteps), str(ExaComm.agent_comm.rank))
        ...
        while True:
            ...
            episode_count_actor = ExaComm.env_comm.bcast(episode_count_actor, root=0)

            # Check if we are done based on the completed episodes
            if episode_count_actor >= workflow.nepisodes:
                break
            logger().info('Rank[{}] - working on episode: {}'.format(ExaComm.agent_comm.rank, episode_count_actor))
            ...
            while done != True:
                if ExaComm.env_comm.rank == 0:
                    ...
                    if self.use_priority_replay:
                        ...
                        loss_data = ind_loss_buffer.pop(ExaComm.agent_comm.rank)
                    ...
                    action = ExaComm.env_comm.bcast(action, 0)
    ...
    def run(self, workflow):
        ...
        if ExaComm.is_agent():
            episode_const = ExaMPIConstant(ExaComm.agent_comm, ExaComm.is_learner() and ExaComm.learner_comm.rank == 0, np.int64, name="Episode_Const")
            epsilon_const = ExaMPIConstant(ExaComm.agent_comm, ExaComm.is_learner() and ExaComm.learner_comm.rank == 0, np.float64, name="Epsilon_Const")

            if self.use_priority_replay:
                # Create windows for priority replay (loss and indices)
                ...
                ind_loss_buffer = self.ind_loss_data_structure(ExaComm.agent_comm, indices_and_loss_for_size, rank_mask=ExaComm.is_actor(),
                                                               length=ExaComm.num_learners, max_model_lag=None, name="Loss_Buffer")

            # Get serialized target weights size
            learner_counter = np.int64(0)
            target_weights = (workflow.agent.get_weights(), learner_counter)
            model_buff = self.target_weight_data_structure(ExaComm.agent_comm, target_weights, rank_mask=ExaComm.is_learner() and
                                                           ExaComm.learner_comm.rank == 0,  length=1, max_model_lag=None, failPush=False, name="Model_Buffer")

            # Get serialized batch data size
            learner_counter = np.int64(0)
            agent_batch = (next(workflow.agent.generate_data()), learner_counter)
            batch_data_exchange = self.batch_data_structure(ExaComm.agent_comm, agent_batch, rank_mask=ExaComm.is_actor(),
        ...
        if ExaComm.is_learner():
            self.learner(workflow, learner_counter, episode_const, epsilon_const, ind_loss_buffer, model_buff, batch_data_exchange, agent_data)
        else:
            self.actor(workflow, episode_const, epsilon_const, ind_loss_buffer, model_buff, batch_data_exchange)
