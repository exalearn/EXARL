...
class ML_RMA(erl.ExaWorkflow):
    ...
    def run(self, workflow):
        ...
        if mpi_settings.is_learner():
            ...
            learner_procs = learner_comm.size
            actor_procs = agent_comm.size - learner_procs
            chunk = int(actor_procs/learner_procs)
            ...
            while episode_count_learner < workflow.nepisodes:
                ...
                #Sai Chenna : assign bunch of actors to each learner to read the RMA window
                if learner_comm.rank == 0:
                    low = learner_procs
                    high = learner_procs + first_offset
                else:
                    low = learner_procs + first_offset+((learner_comm.rank-1)*chunk)
                    high = learner_procs + first_offset+((learner_comm.rank)*chunk)
