...
from exarl.workflows.workflow_vault.sync_learner_ARS import SYNC_ARS
from exarl.utils.profile import PROFILE

class ASYNC_ARS(SYNC_ARS):
    """
    ...
    def run(self, workflow):
        ...
        # workflow  == Exalearner.. (Similar to sync)
        ...
        if ExaComm.is_learner():
            ...
            while self.alive > 0 and self.done_episode < nepisodes:
                ...
                self.debug("Learner DONE", self.alive, self.done_episode, nepisodes)
        else:
            keep_running = True
            while keep_running:
                keep_running = self.ARS_actorWrapper(workflow,nepisodes)
                # self.actor(workflow, nepisodes)
                # print(f"keep_running... {keep_running}",flush=True)
