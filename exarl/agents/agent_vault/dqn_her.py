...
class DQN(erl.ExaAgent):
    ...
    def action(self, state, goal):
        ...
        if rdm <= self.epsilon:
            ...
            total_len = len(state) + len(goal)
            np_state = np.array(concat_state_goal).reshape(1, 1, total_len)
    ...
    def calc_target_f(self, exp):
        state, action, reward, next_state, done, goal = exp
        concat_state_goal = np.concatenate([state, goal])
        concat_next_state_goal = np.concatenate([next_state, goal])
        total_len_state_goal = len(state) + len(goal)
        total_len_next_state_goal = len(next_state) + len(goal)

        np_state = np.array(concat_state_goal).reshape(1, 1, total_len_state_goal)
        np_next_state = np.array(concat_next_state_goal).reshape(1, 1, total_len_next_state_goal)
        expectedQ = 0
        if not done:
            with tf.device(self.device):
