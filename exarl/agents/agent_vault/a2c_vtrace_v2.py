...
class A2Cvtrace_v2(erl.ExaAgent):
    ...
    def update_grad(self, state, action, reward):
        ...
        with tf.GradientTape() as tape1, tf.GradientTape() as tape2:
            ...
            vs = np.zeros(v.shape, dtype=np.float32)
            vs2 = np.zeros(v.shape, dtype=np.float32)

            for s in range(v.shape[0]-1):
                vs[s] = tf.math.add(v[s], tf.math.multiply(self.gamma**s, tf.math.add(TDerror[s], tf.math.multiply(self.gamma, v[s+1]))))

            for s in range(v.shape[0]-1):
                vs2[s] = tf.math.add(TDerror[s], tf.math.multiply(self.gamma, vs[s+1]))
    ...
    def actor_loss(self, probs, actions, vs):
        probability = []
        log_probability = []
        ...
        for pb, v, lpb in zip(probability, vs, log_probability):
            tf.constant(v)
            policy_loss = tf.math.multiply(lpb, v)
