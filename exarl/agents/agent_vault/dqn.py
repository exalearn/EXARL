...
class DQN(erl.ExaAgent):
    ...
    def get_action(self, state):
        ...
        if rdm <= self.epsilon:
            ...
            np_state = np_state.reshape(1, np_state.shape[0])
            with tf.device(self.device):
    ...
    def calc_target_f(self, exp):
        ...
        np_next_state = np_next_state.reshape(1, np_next_state.shape[0])
    ...
    def generate_data(self):
        ...
        if not self.has_data():
            ...
            batch_states = np.zeros((self.batch_size, self.env.observation_space.shape[0]), dtype=self.dtype_observation)
            ...
            batch_states = np.reshape(batch_states, [len(minibatch), dim])
