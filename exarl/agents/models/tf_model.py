...
class Tensorflow_Model(ABC):
    ...
    def _get_device(self):
        ...
        if len(gpus) > 0:
            temp = gpus[ExaComm.agent_comm.rank % len(gpus)]
        else:
            # JS: Fall back on cpu
            temp = cpus[ExaComm.agent_comm.rank % len(cpus)]
        # JS: There is some stupid TF error and this is my workaround
        # return tf.config.PhysicalDevice('/CPU:0', 'CPU'), temp.device_type
        return 
