    ...
    for _ in range(nWorkers):
        ...
        while(len(database[state])+builds[state]>virtuallyConsumed[state]):
            ...
            try:
                ...
                d_alpha  = np.ones(len(list(knownStates.keys())))*0.05
                d_alpha[[x == state for x in knownStates.keys()]]  += 10.0
                for ii in range(d_alpha.shape[0]):
                    loop_state = list(knownStates.keys())[ii]
                    if loop_state in list(knownStates[state].counts.keys()):
                        d_alpha[ii] += knownStates[state].counts[loop_state]
...
class ExaExaaltBayes(gym.Env):
    ...
    def step(self, action):
        ...
        self.crankModel()
        taskList = VE(self.traj, self.knownStates, self.database, self.nWorkers, 0.) # action)
        for i in range(self.nWorkers):
            workerID   = i
